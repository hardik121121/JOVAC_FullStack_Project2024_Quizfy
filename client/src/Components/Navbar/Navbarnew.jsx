...
export const Navbarnew = () => {
  ...
  const logouthandler = () => {
    ...
    if (adminName === "Hardik Arora") {
      toast(`Hardik Arora Successfully Logout `, {
        type: "success",
      });
      dispatch(Logouthandleraction());
    }
  ...
  return (
    <div className="w-11/12 h-24 m-auto flex bg-slate-50 mb-8">
      ...
      <div className="flex justify-center items-center space-x-8 w-8/12">
        <Link to="/" className="text-lg font-medium text-gray-700 hover:text-blue-500">
          Home
        </Link>
        <Link to="/dashboard" className="text-lg font-medium text-gray-700 hover:text-blue-500">
          Dashboard
        </Link>
        <Link to="/profile" className="text-lg font-medium text-gray-700 hover:text-blue-500">
          Profile
        </Link>
      </div>
